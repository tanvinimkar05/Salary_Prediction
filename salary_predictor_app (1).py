...
1.  **First, save the label encoders**: Run the code in the previous cell that saves `label_encoders.pkl`.
2.  **Save this code**: Save the content of this cell into a file named `salary_predictor_app.py` in your Google Drive or Colab environment.
3.  **Install Streamlit**: If you haven't already, run `!pip install streamlit` in a Colab code cell.
4.  **Run the app**: Execute the following command in a Colab code cell:
    `!streamlit run salary_predictor_app.py --server.port 8501 &>/dev/null&`
    Then, you'll usually see a public URL appear. Click on it to open your app in a new browser tab.
""")
